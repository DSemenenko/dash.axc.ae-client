const OutstandingContributionSupport2 = () => {
    return(
        <div className="container">
            <div className="row">
                <div className="col-md-12">
                    <div className="mvp-position">
                        <div className="mvp-screen eight-block-padd">
                            <h1 className="main-title">Outstanding Contribution Q1 2023 Support Services</h1>
                            <div className="img-mvp-block five-img-mvp-block">
                                {/* item 1 */}
                                <div className="item">
                                    <div className="card-mvp">
                                        <div className="img-mvp">
                                            <img src="https://upload-5b6ed3f1470f864e3426fe4c1fe063ed.s3.amazonaws.com/main/7af/7af2a9a036230ccc845ea0d9677013c6/photo1645109294.jpeg" alt="" />
                                        </div>
                                        <div className="mvp-body">
                                            <h3>Jamaica Deri</h3>
                                        </div>
                                    </div>
                                </div>
                                {/* item 2 */}
                                <div className="item">
                                    <div className="card-mvp">
                                        <div className="img-mvp">
                                            <img src="https://upload-5b6ed3f1470f864e3426fe4c1fe063ed.s3.amazonaws.com/main/ea1/ea10255ab03fabc69d7d07feb89c6130/photo_2022-02-17_18-20-30.jpg" alt="" />
                                        </div>
                                        <div className="mvp-body">
                                            <h3>Sonu Sukumaran</h3>
                                        </div>
                                    </div>
                                </div>
                                {/* item 3 */}
                                <div className="item">
                                    <div className="card-mvp">
                                        <div className="img-mvp">
                                            <img src="https://upload-5b6ed3f1470f864e3426fe4c1fe063ed.s3.amazonaws.com/main/859/8595bc2340fd7292a8f43c0db16bfdf4/maria.jpg" alt="" />
                                        </div>
                                        <div className="mvp-body">
                                            <h3>Maria Mahroose</h3>
                                        </div>
                                    </div>
                                </div>
                                {/* item 4 */}
                                <div className="item">
                                    <div className="card-mvp">
                                        <div className="img-mvp">
                                            <img src="https://upload-5b6ed3f1470f864e3426fe4c1fe063ed.s3.amazonaws.com/main/d76/d765af99081c6f705ba05a0bc0ca55b3/photo1667367412.jpeg" alt="" />
                                        </div>
                                        <div className="mvp-body">
                                            <h3>Chloe O`Sullivan</h3>
                                        </div>
                                    </div>
                                </div>
                                {/* item 5 */}
                                <div className="item">
                                    <div className="card-mvp">
                                        <div className="img-mvp">
                                            <img src="https://upload-5b6ed3f1470f864e3426fe4c1fe063ed.s3.amazonaws.com/main/bf3/bf3ad6c5b6d9c48965417752eb59db90/Laeticia+Noronha.JPG" alt="" />
                                        </div>
                                        <div className="mvp-body">
                                            <h3>Laeticia Noronha</h3>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}
export default OutstandingContributionSupport2;